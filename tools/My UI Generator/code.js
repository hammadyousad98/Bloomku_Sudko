...
function frame(parent, name, x, y, width, height) {
...
function image(parent, name, asset, x, y, width, height) {
...
function text(parent, name, value, x, y, width, height, size, align = "CENTER", color = cream) {
...
function sprite(parent, assets, key, name, x, y, width, height) {
    const asset = assets[key];
    if (!asset)
        throw new Error(`Missing gameplay asset: ${key}`);
    return image(parent, name, asset, x, y, width, height);
}
function component(library, assets, key, name, width, height) {
    const asset = assets[key];
    if (!asset)
        throw new Error(`Missing component asset: ${key}`);
...
function placeInstance(parent, master, name, x, y, width, height) {
...
function createLibrary(page, assets) {
...
function createHeader(screen, masters) {
...
function createStats(screen, masters, assets) {
...
function createRules(screen, masters, assets) {
...
function createBoard(screen, masters, assets) {
...
function createPower(screen, masters, assets, name, label, count, x, y, icon) {
    const group = frame(screen, `power.${name}`, x, y, 430, 126);
    placeInstance(group, masters.power_button, `power.${name}.surface`, 0, 0, 430, 126);
    if (icon === "hint")
        placeInstance(group, masters.hint_bulb, "power.hint.icon", 40, 18, 86, 88);
    if (icon === "undo")
        placeInstance(group, masters.undo_arrow, "power.undo.icon", 48, 20, 80, 84);
    if (icon === "solve")
        ["flower_yellow", "flower_pink", "flower_purple"].forEach((key, index) => sprite(group, assets, key, `power.solveRow.icon.${index + 1}`, 42 + index * 48, 36, 50, 50));
    if (icon === "automark") {
...
function createPowers(screen, masters, assets) {
...
function createProgress(screen, masters) {
...
async function generate(backgroundBytes, sources) {
    await loadFonts();
...
figma.ui.onmessage = async (message) => {
    if (message.type === "generate-gameplay") {
        try {
            if (!message.backgroundBytes?.length || !message.spriteAssets)
                throw new Error("Embedded gameplay assets are missing. Run npm run build.");
